...
async function runPythonSnippet(sandbox, datasetId, scriptName, code, args, description) {
...
async function generateSourcePreview(sandbox, sourcePath, datasetId, options = {}) {
    const context = {
        totalRows: 0,
    };
    const countScript = `
...
try:
    ...
    print(str(e))
`;
    const meta = await runPythonSnippet(sandbox, datasetId, "jsonl_count", countScript, [sourcePath], "Counts number of JSONL records with type='row'");
...
try:
    ...
    print(str(e))
`;
    const head = await runPythonSnippet(sandbox, datasetId, "jsonl_head", headScript, [sourcePath, String(headLines)], `Reads the first ${headLines} JSONL row records`);
    context.head = head;
    return context;
}
