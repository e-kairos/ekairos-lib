  ...
  actions: [
    {
      ...
      inputSchema: {
        ...
        properties: {
          data: { type: 'string' as const, required: true, description: 'Datos a procesar' },
        },
      },
      finalize: false,
      // Función no serializable que se ejecuta en el runtime
      execute: async ({ data, contextId }: any) => {
    ...
    },
    {
      ...
      inputSchema: {
        type: 'object' as const,
        properties: {
          result: { type: 'string' as const, description: 'Resultado final' },
        },
      },
      finalize: true,
      execute: async ({ result, contextId }: any) => {
...
const storyEngineInstance = engine.register(exampleStory);
